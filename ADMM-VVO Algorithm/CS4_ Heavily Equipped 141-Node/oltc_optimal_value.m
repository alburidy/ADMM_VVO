...
%==========================
function [Obj_z,u,u_c,flag_z]=oltc_optimal_value(SD,itr,v,vm,rho,lambda,Qc,beta)
...
Objective_z = (rho/2)*norm([vm-(v(SD.oltc_l(:,1))./(0.89375+u*0.00625));(Qc-((u_c*0.03).*v(SD.cb_l).^2))*beta]+lambda,2)^2;

Constraints_z = [1<=u<=33;0<=u_c<=4];
