...
%==========================
function [obj_x,P_loss,vm,Qc,v,theta,flag_x]=vvc(SD,itr,nn,rho,u,u_c,lambda,Gi,Bi,Pd,Qd,Smax,beta)
...
Objective_x = sum(Ploss)+(rho/2)*norm([vm-(v(SD.oltc_l(:,1))./(0.89375+u*0.00625));(Qc-((u_c*0.03).*v(SD.cb_l).^2))*beta]+lambda,2)^2;
