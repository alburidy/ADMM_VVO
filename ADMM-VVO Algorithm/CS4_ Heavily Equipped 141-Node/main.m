...
clear; clc; close all;
%--------------------------
mpc = loadcase('case141');  % Enter any matpower radial case
%--------------------------
...
% rho = 30;
% beta=1e-1; % SCB Scaling Factor
lambda = [repmat(1e-1,SD.tpn,1);repmat(1e-1,SD.cbn,1)];
rho = 20;
beta=1e-1; % SCB Scaling Factor
%--------------------------
...
while (test_1>epison_1 || test_2>epison_2) && itr<=itr_max
    ...
    [obj_x,P_loss,vm,Qc,v,theta,flag_x]=vvc(SD,itr,nn,rho,u,u_c,lambda,Gi,Bi,Pd,Qd,Smax,beta);
    % The slack update step:
    [Obj_z,u_n,u_c_n,flag_z]=oltc_optimal_value(SD,itr,v,vm,rho,lambda,Qc,beta);
    ...
    lambda_u = lambda+value([vm-(v(SD.oltc_l(:,1))./(0.89375+u_n*0.00625));(Qc-((u_c_n*0.03).*v(SD.cb_l).^2))*beta]);
    ...
    fprintf(['%6.0f %12.5f %12.4f %8.4f %10.3f %15f %13.0f      |',...
        repmat(' %2.0f ',1, SD.tpn+SD.cbn),'\n']',itr,obj_x,max(v),min(v),rho,test_1,test_2,[u;u_c]');
    ...
    history.convergence(itr,:)=value([vm-(v(SD.oltc_l(:,1))./(0.89375+u*0.00625));(Qc-((u_c*0.03).*v(SD.cb_l).^2))*beta]);
...
if  (itr<=itr_max && flag_x==0 && flag_z==0)
    ...
    mpc.bus(SD.cb_l,6)=(u_c.*0.03)*mpc.baseMVA;
